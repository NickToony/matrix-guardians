...
while (!ds_priority_empty(openSet)) {
	...
	for (var i = 0; i <= 3; i ++) {
		...
		// Check if it's solid!
		if (neighbour == noone || (neighbour.solid && !neighbour.diggable)) {
		...
		if (neighbour.object_index == objBlockTile) {
			nodeGscore += 25 + irandom(25);	
		}
		if (neighbour.object_index == objDirtBlock) {
			nodeGscore += 5 + irandom(5);	
		}
		if (neighbour.object_index == objMetalBlock) {
			nodeGscore += 100;	
		}
